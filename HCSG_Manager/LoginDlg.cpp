...
void CLoginDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK_INC_IP, m_CButtonChkIncIP);
	DDX_Control(pDX, IDC_COMBO_SORT, m_CComboBoxSort);
}
...
BEGIN_MESSAGE_MAP(CLoginDlg, CDialogEx)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_SORT, &CLoginDlg::OnCbnSelchangeComboSort)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST_ACCOUNT, &CLoginDlg::OnLvnColumnclickListAccount)
END_MESSAGE_MAP()
...
void CLoginDlg::UpdataAcc()
{
	...
	if (Common::GetGameAccFormDB() == FALSE) return;
...
void CLoginDlg::OnBnClickedButtonFindNextAcc()
{
	// TODO: 在此添加控件通知处理程序代码
	CString strKey;
	int iColumnNum, iRowCount,iMaxCheckColumn;
	char chTemp[32];
	int ListIndex = 0;
	...
	// 是否需要把ip地址包含进搜索范围搜索
	iMaxCheckColumn = 2;  // 只查找账户名和角色名
	int aaa = m_CButtonChkIncIP.GetCheck();
	if (this->IsDlgButtonChecked(IDC_CHECK_INC_IP) == BST_CHECKED)
	{
		iMaxCheckColumn = iColumnNum;
	}
	...
	{
		memset(chTemp, 0 ,32);
		for (int i = 0; i<iMaxCheckColumn; i++)
		{
			m_CListCtrlAccount.GetItemText(j,i,chTemp, 32);
			if(strstr(chTemp,strKey))
			{
...
void CLoginDlg::OnBnClickedAddAccount()
{
	...
	// UpdataAcc();  // 账户多了后每次刷新都很慢，所以改成手动刷新，按需执行  -- 实际还是效率不行
}
...
void CLoginDlg::OnCbnSelchangeComboSort()
{
	// TODO:  在此添加控件通知处理程序代码
}
...
void CLoginDlg::OnLvnColumnclickListAccount(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	int len = m_CListCtrlAccount.GetItemCount();
	for (int i = 0; i < len; i++)
	{
		m_CListCtrlAccount.SetItemData(i, i);
	}

	DATA data;
	data.col = dwSelColID;//取列
	data.plist = &m_CListCtrlAccount;//取列表指针
	m_CListCtrlAccount.SortItems(CompareFunc, (LPARAM)&data);

	//m_CListCtrlAccount.SortItems(CompareFunc, (LPARAM)(&m_CListCtrlAccount));
}
