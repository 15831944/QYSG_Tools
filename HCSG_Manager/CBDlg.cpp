...
void CCBDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_MAP_CB, m_CEditMapPath);
}
...
BEGIN_MESSAGE_MAP(CCBDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_MAP_CB, &CCBDlg::OnEnChangeEditMapCb)
	ON_BN_CLICKED(IDC_BUTTON_GET_AWARDS_LIST, &CCBDlg::OnBnClickedButtonGetAwardsList)
	ON_BN_CLICKED(IDC_BUTTON_SEDN_AWARDS, &CCBDlg::OnBnClickedButtonSednAwards)
END_MESSAGE_MAP()
...
BOOL CCBDlg::OnInitDialog()
{
	...
	m_CEditMapPath.SetWindowText(Common::m_CbMapPath);
...
void CCBDlg::GetHores()
{
	...
	strPathName = Common::ServerPath + Common::m_CbMapPath + "\\history\\history_0351.txt";
	CString szLine = "";
	CString strGap = _T(",");
...
LRESULT CCBDlg::DoSaveConfig(WPARAM wParam, LPARAM lParam)
{
	...
	TRY
	{
		...
		if( database.OpenEx(sSql,CDatabase::noOdbcDialog) )
		{
			...
			sSql = " CREATE TABLE 赤壁英雄奖励 ( ";
			strInsert = " INSERT INTO 赤壁英雄奖励 ( " ;
			//获得列标题名称
		...
		// 关闭Excel表格文件
		database.Close();
		Common::Log(Info, "保存赤壁英雄奖励配置成功！");
	}
	CATCH_ALL(e)
	{
		//错误类型很多，根据需要进行报错。
		Common::Log(Error, "赤壁奖励配置保存失败。");
	}
	END_CATCH_ALL;
...
BOOL CCBDlg::DoLoadConfig(CString strFilePath)
{
	...
	sSql.Format("SELECT 起始排名 , 结束排名 , 物品ID1 , 物品名1 , 数量1 , 物品ID2 , 物品名2 , 数量2 ,"
		" 物品ID3 , 物品名3 , 数量3 , 物品ID4 , 物品名4 , 数量4 , 物品ID5 , 物品名5 , 数量5 FROM 赤壁英雄奖励");
...
void CCBDlg::StartTimer(CStringArray *StartTimes)
{
	...
	if(i < StartTimes->GetCount())
	{
		...
		Common::Log(Info, "下次赤壁时间：" + strWarStartTime);
		int  tmp_h, tmp_mm;
...
void CCBDlg::OnBnClickedButtonExe()
{
	...
	if(ExeStart)
	{
		...
		Common::Log(Info, "赤壁奖励检测停止");
		ExeStart = FALSE;
	}
	else
	{
		GetDlgItem(IDC_BUTTON_EXE)->SetWindowText("停止自动模式");
		Common::Log(Info, "赤壁奖励检测开始");
		ExeStart = TRUE;
	}
}
	...
	case TIMER2:
		{
			SetStateStatic(FALSE);
			KillTimer(TIMER2);
			Common::Log(Info, "赤壁之战结束！");
			SetTimer(TIMER3, 5 * 60 * 1000, 0); // 推迟5分钟发奖
	...
	for(int i = 0; i < iRowCountAcc; i++)
	{
		...
		for (int j=0 ; j<iRowCountConfig ; j++ )
		{
			...
			if(_ttoi(strTemp) <= nRanking && _ttoi(strTemp2) >= nRanking)
			{
				...
				{
					Common::Log(Error, "发放赤壁奖励失败");
				}
			}
		}
	}
	Common::Log(Info, "赤壁奖励发放结束");
}
...
void CCBDlg::OnEnChangeEditMapCb()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
	...
	// TODO:  在此添加控件通知处理程序代码
	m_CEditMapPath.GetWindowText(Common::m_CbMapPath);
	Common::SaveConfig();
}
...
void CCBDlg::OnBnClickedButtonGetAwardsList()
{
	// TODO:  在此添加控件通知处理程序代码
	GetHores();
}
...
void CCBDlg::OnBnClickedButtonSednAwards()
{
	// TODO:  在此添加控件通知处理程序代码
	PaymentOfAwards();
}
