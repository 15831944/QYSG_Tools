...
#define VIP_LEVEL 30
#define MODLE_LEVEL	20

#define CONFIG_FILE_PATH ".\\Profile\\"
#define CONFIG_FILE "配置.ini"
#define VIP_CONFIG_FILE "VIP配置.ini"
#define MODLE_CONFIG_FILE "模版配置.ini"
...
public:
	...
	static CString TableAccountName;
	static CString TableXbName;
	...
	static BOOL GetHdSn();
	static BOOL GetCpuId();
	static CString GetMachineCode();
	static BOOL LicenseDecodeAndCheck();
	static int ConnectAndGetData(TimeData *td);
	static int ParseTimeString(TimeData *td, char* buf);
	static int CheckFirstEl(char *buf);
	static void GetNetTime(CString csDate);
	static CString ConvertTime(unsigned long ulTime);
